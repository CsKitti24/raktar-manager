...
class OrderItemSchema(Schema):
    ...
    supplied_quantity = Integer(allow_none=True)
...
class OrderResponseSchema(Schema):
    ...
    created_at = DateTime()
    updated_at = DateTime()
    estimated_delivery_at = DateTime(allow_none=True)
    supplier_notes = String(allow_none=True)
    address = Nested(AddressResponseSchema)
    items = List(Nested(OrderItemSchema))

class SupplierOrderItemSchema(Schema):
    id = Integer(required=True)
    supplied_quantity = Integer(required=True)

class SupplierFormRequestSchema(Schema):
    estimated_delivery_at = DateTime(required=True)
    supplier_notes = String(allow_none=True)
    items = List(Nested(SupplierOrderItemSchema), required=True)
